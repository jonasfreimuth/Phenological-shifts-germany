...
# additionally save faceted plots of diagnostic plots 
...
slopes_all_tab <- slopes_all_sum %>% 
    ...
    mutate(across(matches("frac_negative"), ~ .x * 100)) %>% 
    mutate(across(where(is.numeric), round, 2)) %>% 
    
    select(
      Group                                    = id.grp,
      `n species`                              = n_spec_year,
      `Average time slope`                     = slope_mean_year,
      `Time slope SEM`                         = slope_sem_year,
      `Percentage advancing\nover time`        = slope_frac_negative_year,
      `Average temperature slope`              = slope_mean_temp,
      `Temperature slope SEM`                  = slope_sem_temp,
      `Percentage advancing\nwith temperature` = slope_frac_negative_temp) %>% 
...
asyn_tab <- int_slopes_asyn_sum %>% 
    ...
    select(
      `Group`                                 = group,
      `n interactions`                        = n_int,
      `n plant species`                       = n_plant,
      `n pollinator species`                  = n_poll,
      `Average asynchrony\ntime slope`        = slope_year_asyn_mean,
      `Time slope SEM`                        = slope_year_asyn_sem,
      `Average asynchrony\ntemperature slope` = slope_temp_asyn_mean,
      `Temperature slope SEM`                 = slope_temp_asyn_sem) %>% 
...
# save the docx file
print(doc_file, "plots/tables.docx")
