...
int_slope_asyn <- int_data %>%
  apply(1, function(int_data_row) {
    ...
    return(data.frame(
      ...
      slope_temp_asyn  = slope_all_plant$slope_temp -
        slope_all_poll $slope_temp,
      
      # compute their SEs
      slope_temp_asyn_std_err  = sqrt(
        slope_all_plant$slope_std_err_temp  ^ 2 +
          slope_all_poll$slope_std_err_temp ^ 2),
      
      
      # same for intercepts
      intcp_temp_asyn  = slope_all_plant$intercept_temp - 
        slope_all_poll $intercept_temp,
      
      # compute their SEs
      intcp_temp_asyn_std_err  = sqrt(
        slope_all_plant$intercept_std_err_temp  ^ 2 +
          slope_all_poll$intercept_std_err_temp ^ 2),
      ...
      slope_year_asyn  = slope_all_plant$slope_year -
        slope_all_poll $slope_year,
      
      # compute their SEs
      slope_year_asyn_std_err  = sqrt(
        slope_all_plant$slope_std_err_year  ^ 2 +
          slope_all_poll$slope_std_err_year ^ 2),
      
      
      # same for intercepts
      intcp_year_asyn  = slope_all_plant$intercept_year - 
        slope_all_poll $intercept_year,
      
      # compute their SEs
      intcp_year_asyn_std_err = sqrt(
        slope_all_plant$intercept_std_err_year  ^ 2 +
          slope_all_poll$intercept_std_err_year ^ 2)))
...
aov_asyn_temp <- aov(data = int_slope_asyn, slope_temp_asyn ~ group,
...
aov_asyn_year <- aov(data = int_slope_asyn, slope_year_asyn ~ group,
...
int_slope_asyn_sum <- int_slope_asyn %>% 
  group_by(group) %>% 
...
ggplot() +
  ...
  # raw data
  geom_pointrange(data = int_slope_asyn,
                  aes(x    = group,
                      y    = slope_temp_asyn,
                      ymin = slope_temp_asyn - slope_temp_asyn_std_err,
                      ymax = slope_temp_asyn + slope_temp_asyn_std_err),
                  col = col.pt,
                  position = position_jitter()) +
  
  # error bars
  geom_errorbar (data = int_slope_asyn_sum,
                 aes(x    = group,
                     ymin = slope_temp_asyn_mean - slope_temp_asyn_se,
                     ymax = slope_temp_asyn_mean + slope_temp_asyn_se,
                     col  = group)) +
  
  # mean data
  geom_point    (data = int_slope_asyn_sum,
                 aes(x   = group,
...
ggplot() +
  ...
  geom_pointrange(data = int_slope_asyn,
                  aes(x    = group,
                      y    = slope_year_asyn,
                      ymin = slope_year_asyn - slope_year_asyn_std_err,
                      ymax = slope_year_asyn + slope_year_asyn_std_err),
                  col = col.pt,
                  position = position_jitter()) +
  ...
  geom_errorbar (data = int_slope_asyn_sum,
                 aes(x    = group,
                     ymin = slope_year_asyn_mean - slope_year_asyn_se,
                     ymax = slope_year_asyn_mean + slope_year_asyn_se,
                     col  = group)) +
  ...
  labs(title = "Interaction asynchrony shifts",
       subtitle = "over time",
       x = "Group",
       y = "Asynchrony shift [days / year] (\u00B1 SE)") +
