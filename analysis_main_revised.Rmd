...
slopes_temp_sum <- slopes_temp %>% 
  ...
  summarise(n_spec              = length(slope),
            ...
            slope_sd            = sd(slope),
            slope_se            = sqrt(sum(slope_std_err ^ 2)) / length(slope),
            slope_ci_min        = slope_mean - slope_se * 1.96,
            slope_ci_max        = slope_mean + slope_se * 1.96,
...
slopes_temp %>% 
  ...
  geom_pointrange(aes(x    = reorder(species,  -slope),
                      y    = slope,
                      ymin = slope - slope_std_err,
                      ymax = slope + slope_std_err, col = id.grp)) +
...
ggplot() +
  ...
  # raw data
  geom_pointrange(data = slopes_temp,
                  aes(x    = id.grp,
                      y    = slope,
                      ymin = slope - slope_std_err,
                      ymax = slope + slope_std_err),
                  col = col.pt,
...
ggplot() +
  ...
  # raw data
  geom_pointrange(data = slopes_year,
                  aes(x    = id.grp,
                      y    = slope,
                      ymin = slope - slope_std_err,
                      ymax = slope + slope_std_err),
...
ggplot() +
  ...
  geom_errorbarh (data = slopes_all_sum,
                  aes(y    = slope_mean_temp,
                      xmin = slope_mean_year - slope_se_year,
                      xmax = slope_mean_year + slope_se_year,
                      col  = id.grp
                      )) +
  ...
  geom_errorbar  (data = slopes_all_sum,
                  aes(x    = slope_mean_year,
                      ymin = slope_mean_temp - slope_se_year,
                      ymax = slope_mean_temp + slope_se_year,
                      col  = id.grp))  +
  ...
  # improve axis lables 
  labs(x = "Shift over time [days / year] (\u00B1 SE)",
       y = "Shift with temperature [days / \u00B0C] (\u00B1 SE)") +
...
int_slope_diff <- int_data %>% 
  apply(1, function(int_data_row) {
    ...
    return(data.frame(
      ...
      # compute the difference between both time slopes
...
aov_int_temp <- aov(data = int_slope_diff, slope_temp_diff ~ group)
...
aov_int_year <- aov(data = int_slope_diff, slope_year_diff ~ group)
...
int_slope_diff_sum <- int_slope_diff %>% 
  group_by(group) %>% 
...
ggplot() +
  ...
  # raw data
  geom_jitter   (data = int_slope_diff,
                 aes(x = group,
                     y = slope_temp_diff),
                 col = col.pt) +
  
  # error bars
  geom_errorbar (data = int_slope_diff_sum,
                 aes(x    = group,
  ...
  # mean data
  geom_point    (data = int_slope_diff_sum,
                 aes(x   = group,
...
int_slope_syn <- int_data %>% 
  apply(1, function(int_data_row) {
    ...
    out <- data.frame(
      ...
      # extract their intercept for temp
      intcp_temp_plant = slope_all_plant$intercept_temp,
      intcp_temp_poll  = slope_all_poll $intercept_temp,
      ...
      # extract their intercept for time
      intcp_year_plant = slope_all_plant$intercept_year,
      intcp_year_poll  = slope_all_poll $intercept_year)
    
    # compute asynchrony for temp and time
    asyn_temp <- (out$intcp_temp_plant + out$slope_temp_plant * temp_vec) -
      (out$intcp_temp_poll + out$slope_temp_poll * temp_vec)
    
    asyn_year <- (out$intcp_year_plant + out$slope_year_plant * year_vec) -
      (out$intcp_year_poll + out$slope_year_poll * year_vec)
    
    # get the slope and intercept for both
    asyn_coef_temp <- coef(lm(asyn_temp ~ temp_vec))
    asyn_coef_year <- coef(lm(asyn_year ~ year_vec))
    
    # add those to the df
    out$slope_temp_asyn <- asyn_coef_temp[["temp_vec"   ]]
    out$intcp_temp_asyn <- asyn_coef_temp[["(Intercept)"]]
    
    out$slope_year_asyn <- asyn_coef_year[["year_vec"   ]]
    out$intcp_year_asyn <- asyn_coef_year[["(Intercept)"]]
...
ggplot() +
  ...
  # raw data
  geom_pointrange(data = slopes_temp_polldep,
                  aes(x    = PollDep,
                      y    = slope,
                      ymin = slope - slope_std_err,
                      ymax = slope + slope_std_err),
...
ggplot() +
  ...
  # raw data
  geom_pointrange(data = slopes_year_polldep,
                  aes(x    = PollDep,
                      y    = slope,
                      ymin = slope - slope_std_err,
                      ymax = slope + slope_std_err),
