...
mtemp <- glmmTMB(doy ~ temp + lat + long + temp * id.grp + (temp | species),
          family = gaussian,
          data = dat.occ)
...
saveRDS(mtemp, file = paste("data/mtemp_model_cent_",
...
mtime <- glmmTMB(doy ~ year + lat + long + year * id.grp + (year | species),
          family = gaussian,
          data = dat.occ)
...
saveRDS(mtime, file = paste("data/mtime_model_cent_",
                            format(Sys.time(), format = "%Y%m%d_%H%M"), ".rds",
                            sep = ""))
...
saveRDS(mtime, file = paste("data/mtime_model_",
                            format(Sys.time(), format = "%Y%m%d_%H%M"), ".rds",
                            sep = ""))
