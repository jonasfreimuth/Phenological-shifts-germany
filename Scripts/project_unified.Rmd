...
#set threshold for min # of records per decade
thr.dec.rec <- 5
...
#set cutoff for earliest year of records
cutoff.year <- 1960

#set cutoff for decadal records
cutoff.dec <- 1980

# force prunning? (necessary after changing earliest year cutoff)
force.pruning <- FALSE

# force calculation mean doys? (better to only set it to FALSE if you're 
# really impatient)
force.averaging <- FALSE

# give level of addtional plots
# full -> do everything
# base -> only base analysis (or just leave it blank for now)
# or a vector of desired options
# one of 
# - spec.int.slopes: plots interactions on species level (mean.doy vs time/temp)
# - traits: analyses all plant traits and their influence on response over time and temp
# - all.spec: plots all species reaction to time in one plot
# - int.spec.dec: same as spec int slope but on the level of difference of doy and in decades
# - data.quality.assesment: plots all occurrence records of plants and pollinators in one each
#                           (not included in full, requires more than 8 GB of RAM (right now at least))
...
ypos <- function(x, ..., frac = 0.3) {
  ...
  return(max(x, na.rm = TRUE) + abs(max(x, na.rm = TRUE)) * frac)
...
slopes <- function(data, formula = mean.doy ~ year,
  ...
  #   } else if (taxon == "order") {
  #     stat.out <-
  #       select(stat.out, everything(), -species, -genus, -family)
  #     
  #   } else if (taxon == "phylum") {
  #     stat.out <-
  #       select(stat.out,
...
if (all(file.exists(
  ...
  if (
    # check if last keys is younger than the file created after downloading occurrences
    ...
    file.mtime(here("data", "occurrences_full_pruned.csv")) >
    file.mtime(here("data", "occurrences_species_yearly_mean_doy_pruned.csv"))) {
...
ggplot() +
  ...
  #             alpha = .7) +
  theme_minimal() +
  labs(title = "Linear Relationship Between between year and\nCollection Day for each Group",
       x = "Year",
       y = "DOY")
...
ggplot() +
  ...
  #             alpha = .7) +
  theme_minimal() +
  labs(title = "Linear Relationship Between between temperature and\nCollection Day for each Group",
...
ggplot(data = dat.occ, 
  ...
  geom_abline(intercept = (coef.time[1,1] + coef.time[3,1]),
              slope = (coef.time[2,1] + coef.time[4,1]), size = 1.5, col = "#01bec3") +
   labs(x = "Year", 
       y = "DOY", 
...
ggplot(data = dat.occ, 
  ...
  geom_point(aes(alpha = n.rec)) +
  geom_abline(intercept = coef.temp[1,1], slope = coef.temp[2,1], size = 1.5, col = "#f97970") +
  geom_abline(intercept = (coef.temp[1,1] + coef.temp[3,1]),
              slope = (coef.temp[2,1] + coef.temp[4,1]), size = 1.5, col = "#01bec3") +
...
ggplot() +
  ...
  theme(legend.position = "none")+
  labs(x = "Year", y = "DOY",
       title = "Linear Relationship between between year and Collection Day\nfor plant orders")
...
```{r Shifts of duration, fig.width=10,fig.height=7}
...
stat.spec.dur <- bind_cols(select(slopes(dat.occ.dur.dec, duration ~ decade, time_var = decade),
                                  -rsquared, -fstat),
                           rename_all(select(slopes(dat.occ.dur.dec, min.doy ~ decade, time_var = decade),
                                  slope, intercept, pval, fdr.pval, df, ci.min, ci.max),
                                  function(x) str_glue("{x}.first")),
                           rename_all(select(slopes(dat.occ.dur.dec, max.doy ~ decade, time_var = decade),
                                  slope, intercept, pval, fdr.pval, df, ci.min, ci.max),
                                  function(x) str_glue("{x}.last"))) %>%
...
slope_dec_dur <- dur_slope_plot(xlab = "Group", ylab = "Mean slope [days/year] (\u00B1 95% CI)",
               title = "Group shift of decadal mean\nduration of activity over time")

slope_dec_first <- dur_slope_plot(y = slope.first,
...
slope_dec_last <- dur_slope_plot(y = slope.last,
...
slope_dec_diff <- dur_slope_plot(y = slope.diff,
...
gridExtra::grid.arrange(slope_dec_first, slope_dec_last, slope_dec_diff, slope_dec_dur)
...
```{r}
# get slopes for first, last doy and their difference (duration) into a df
stat.spec.dur.2k <- bind_cols(select(slopes(dat.occ.2k, duration ~ year, thr = 2),
                                  -rsquared, -fstat),
                           rename_all(select(slopes(dat.occ.2k, min.doy ~ year, time_var = year, thr = 2),
                                  slope, intercept, pval, fdr.pval, df, ci.min, ci.max),
                                  function(x) str_glue("{x}.first")),
                           rename_all(select(slopes(dat.occ.2k, max.doy ~ year, time_var = year, thr = 2),
                                  slope, intercept, pval, fdr.pval, df, ci.min, ci.max),
                                  function(x) str_glue("{x}.last"))) %>%
  # also calculate difference of slope of last and first doy
  mutate(slope.diff = abs(slope.last) - abs(slope.first))
...
slope_dur <- dur_slope_plot(y = slope,
               ymeta = mean.slope,
               ymin = ci.min,
               ymax = ci.max,
               ...
               title = str_glue("Group shift of yearly mean
                                duration of flight over time ({thr.2k.min} - {thr.2k.max})"))

slope_first <- dur_slope_plot(y = slope.first,
               ymeta = mean.slope.first,
               ymin = ci.min.first,
               ymax = ci.max.first,
               data = stat.spec.dur.2k,
...
slope_last <- dur_slope_plot(y = slope.last,
               ymeta = mean.slope.last,
               ymin = ci.min.last,
               ymax = ci.max.last,
               data = stat.spec.dur.2k,
...
slope_diff <- dur_slope_plot(y = slope.diff,
               ymeta = mean.slope.diff,
               ymin = ci.min.diff,
               ymax = ci.max.diff,
               data = stat.spec.dur.2k,
               ...
               xlab = "Group",
               ylab = "Mean slope [days/year] (\u00B1 95% CI)",
               title = str_glue("Group differences of shift of yearly mean
...
# store results in df
cor.tests <- lapply(unique(stat.spec.both$id.grp), function(x, data = stat.spec.both) {
...
cor.tests <- do.call(rbind, cor.tests) %>%
  mutate(pval.sig = cut(pval, breaks = c(0, 0.001, 0.01, 0.05, 0.1, 1), labels = c("***", "**", "*", ".", " "),
                      right = FALSE)) %>% 
...
cat("It may not be possible to calculate slopes for species in this timeframe.\nAs a result, group sizes may differ.")

# calculate species slopes
stat.spec.2k.time <- slopes(dat.occ.2k, thr = 2)
stat.spec.2k.temp <- slopes(dat.occ.2k, formula = mean.doy ~ y_mean_temp, thr = 2)
...
dat.int <- read.csv(here("static_data", "plant_pollinator_interactions_2018.csv"), stringsAsFactors = FALSE) %>%
  rename(poll = POLLINATOR_NAME, plant = PLANT_NAME) %>%
  
  #add the plant trait data to the interactions
  left_join(fread(here("static_data", "bioflor_traits.csv")), by = c("plant" = "species"))%>%
...
for (i in seq(1, nrow(dat.int.year))) {
  ...
  dat.i <- rbind(filter(dat.occ, species == as.character(dat.int.year$poll[i])) %>% droplevels(),
                 filter(dat.occ, species == as.character(dat.int.year$plant[i])) %>% droplevels())
  
  # if speciefied plot the phenological changes of the interaction
  if (any(c("full", "spec.int.slopes", "spec.int.slopes.time") %in% opts)) {
    
    #make sure directory exists
    dir.check(here("plots/interactions/time"))
    
    png(here("Plots/Interactions/time", paste0(i, "_", paste(unique(dat.i$species), collapse = ""),
                                               ".png")),
        width = 1500, height = 1000)
    
    print(
      ggplot(dat.i, aes(x = year, y = mean.doy, col = kingdom)) +
        geom_point(aes(size = n.rec)) +
        geom_smooth(method = "lm", size = 1.5) +
        labs(title = toString(unique(dat.i$species))) +
  ...
  ancov.i <- anova(lm(mean.doy ~ year, dat.i), lm(mean.doy ~ year * kingdom, dat.i))
  ...
  dat.int.i <- dat.i %>%
    summarise(group = dat.int.year$Group[i],
              ...
              plant.slope = sum.lm.i[["coefficients"]][2,1] + sum.lm.i[["coefficients"]][4,1],
...
write.csv(dat.int.time, here("Data", "pla_poll_interactions_time.csv"), row.names = FALSE)
...
for (i in seq(1, nrow(dat.int.year))) {
  ...
  # if speciefied plot the phenological changes of the interaction
  if (any(c("full", "spec.int.slopes", "spec.int.slopes.temp") %in% opts)) {
    
    # check if the directory is present
    # if it doesn't exist yet, create the interaction subdirectory
    dir.check(here("plots/interactions/temp"))
    
    png(here("Plots/Interactions/temp", paste0(i, "_", paste(unique(dat.i$species), collapse = ""),
                                               ".png")),
        width = 1500, height = 1000)
    
    print(
      ggplot(dat.i, aes(x = y_mean_temp, y = mean.doy, col = kingdom)) +
        geom_point(aes(size = n.rec)) +
        geom_smooth(method = "lm", size = 1.5) +
        labs(title = toString(unique(dat.i$species))) +
        coord_cartesian(xlim = c(min(dat.occ$y_mean_temp), max(dat.occ$y_mean_temp)),
                        ylim = c(0, 365)) +
        theme(axis.title = element_text(size = 40),
              axis.text = element_text(size = 40),
              axis.text.x = element_text(angle = 30, hjust = 1),
  ...
  #run an ancova to test for a difference in the slopes of plants and pollinators
  ancov.i <- anova(lm(mean.doy ~ y_mean_temp, dat.i), lm(mean.doy ~ y_mean_temp * kingdom, dat.i))
...
dat.occ.dec <- fread(here("data", "occurrences_species_decadal_mean_doy_pruned.csv"), showProgress = FALSE) %>%
...
for (i in seq(1, nrow(int.dec))) {
  ...
  sum.i.time <- summary(lm(data = int.i.mean, formula = doy.diff ~ decade))
  
  #extract model output
  stat.int.i.time <- data.frame(group = int.dec$Group[i],
                           poll = int.dec$poll[i],
                           plant = int.dec$plant[i],
                           yintercept = as.data.frame(sum.i.time$coefficients)$Estimate[1],
                           doy.diff.slope = as.data.frame(sum.i.time$coefficients)$Estimate[2],
  ...
  #generate model for doy differences over time
  sum.i.temp <- summary(lm(data = int.i.mean, formula = doy.diff ~ y_mean_temp))
  
  #extract model output
  stat.int.i.temp <- data.frame(group = int.dec$Group[i],
                           poll = int.dec$poll[i],
                           plant = int.dec$plant[i],
                           yintercept = as.data.frame(sum.i.temp$coefficients)$Estimate[1],
                           doy.diff.slope = as.data.frame(sum.i.temp$coefficients)$Estimate[2],
  ...
  if (any(c("full", "int.spec.dec") %in% opts)) {
    ...
    print(
      ggplot(int.i.mean, aes(x = decade, y = doy.diff)) +
        geom_line(size = 1.5) +
        geom_point(size = 4) +
        geom_smooth(method = "lm") +
    ...
    print(
      ggplot(int.i.mean, aes(x = y_mean_temp, y = doy.diff)) +
        geom_line(size = 1.5) +
        geom_point(size = 4) +
        geom_smooth(method = "lm") +
        labs(title = paste(unique(int.i.mean$poll),
...
int.mean.time <- bind_rows(
  ...
  int.spec.dec %>%
    ...
    summarise(mean.doy.diff = mean(doy.diff),
              ci.min.doy.diff = ci.min(doy.diff),
              ci.max.doy.diff = ci.max(doy.diff),
...
eqs.doy.diff$xintercept[1] <- xintercpt(eqs.doy.diff$yintercept[1], eqs.doy.diff$slope[1])
...
for (i in seq(2, nrow(eqs.doy.diff) - 1)) {
  ...
  eqs.doy.diff$yintercept[i] <- coeffs.doy.diff$Estimate[1] + coeffs.doy.diff$Estimate[1+i]
  
  # postition of the slope is always the number of groups
  # plus the current group
  eqs.doy.diff$slope[i] <- coeffs.doy.diff$Estimate[2] + coeffs.doy.diff$Estimate[nrow(eqs.doy.diff) - 1 + i]
  
  # calculate x intercept (i.e. when the average trend reverses)
  eqs.doy.diff$xintercept[i] <- xintercpt(eqs.doy.diff$yintercept[i], eqs.doy.diff$slope[i])
...
eqs.ins.earl$xintercept[1] <- xintercpt(eqs.ins.earl$yintercept[1], eqs.ins.earl$slope[1])
...
for (i in seq(2, nrow(eqs.ins.earl) - 1)) {
  ...
  # postition of the intercept for the other groups is always 
  # the intercept plus the position of the current group
  eqs.ins.earl$yintercept[i] <- coeffs.ins.earl$Estimate[1] + coeffs.ins.earl$Estimate[1+i]
  
  # postition of the slope is always the number of groups
  # plus the current group
  eqs.ins.earl$slope[i] <- coeffs.ins.earl$Estimate[2] + coeffs.ins.earl$Estimate[nrow(eqs.ins.earl) - 1 + i]
  
  # calculate x intercept (i.e. when the average trend reverses)
  eqs.ins.earl$xintercept[i] <- xintercpt(eqs.ins.earl$yintercept[i], eqs.ins.earl$slope[i])
...
ggplot() +
  geom_point(data = int.spec.dec,
             aes(y_mean_temp, doy.diff,
                 col = group),
             alpha = .2) +
  geom_line(data = int.spec.dec,
            aes(y_mean_temp, doy.diff, group = id,
                col = group),
            alpha = .1) +
  geom_errorbar(data = int.mean.temp,
                aes(y_mean_temp, ymin = ci.min.doy.diff,
                    ymax = ci.max.doy.diff,
                    col = group),
                size = 1.2) +
  geom_point(data = int.mean.temp,
             aes(y_mean_temp, mean.doy.diff, col = group,
                 pch = group),
             size = 2) +
...
```
  ...
  Distributions of Slopes of decadal mean DOY differences between groups:  

```{r Interaction difference slope meta stats}

#calculate N, mean and sd of doy.diff.slopes
stat.int.meta <- stat.int.time %>%
  group_by(group, decades) %>%
  summarise(
    N.doy.diff.slope = length(doy.diff.slope),
    mean.doy.diff.slope = mean(doy.diff.slope),
    se.doy.diff.slope = sd(doy.diff.slope)/sqrt(length(doy.diff.slope)),
    sd.doy.diff.slope = sd(doy.diff.slope),
    ci.min = ci.min(doy.diff.slope),
    ci.max = ci.max(doy.diff.slope),
    negfrac = sum(doy.diff.slope < 0) / length(doy.diff.slope),
...
# ggplot(stat.int, aes(x = group, y = doy.diff.slope)) +
#   geom_boxplot() +
#   labs(x = "Group", y = "Slope of Difference of DOY") + 
#   geom_text(data = stat.int.meta, 
#             aes(x = group, y = 7, label = paste0("n =  ", N.doy.diff.slope)),
#             col = "gray32") +
#   # scale_y_reverse() + 
#   theme_minimal()
...
ggplot(data = int.spec.dec) +
  geom_point(aes(x = group, y = doy.diff, col = decade),
                 position = position_jitter(width = .4),
             alpha = .3) +
  scale_color_gradientn(colors = palette(100)) +
  labs(x = "Group", y = "DOY difference") +
  theme_minimal()
...
ggplot() +
  ...
  geom_text(data = stat.int.meta, 
            aes(x = group, y = 5.5, label = paste0("n =  ", N.doy.diff.slope)),
            col = "gray32") +
  labs(x = "Group",
       y = "Mean Slope of Difference of DOYs [days/decade] (\u00B1 95% CI)",
       title = "Mean Slope of Difference of DOYs over Groups") +
  theme_minimal() +
  theme(legend.position = "none") 
...
mod.int.bee <- aov(data = filter(stat.int.time, group == "Bee"), doy.diff.slope ~ decades)
...
mod.int.butterfly <- aov(data = filter(stat.int.time, group == "Butterfly"), doy.diff.slope ~ decades)
...
mod.int.hoverfly <- aov(data = filter(stat.int.time, group == "Hoverfly"), doy.diff.slope ~ decades)
...
cat("\nAnova Mean Slope of Difference of DOYs (all decades):\n\n")

mod.int.all <- aov(data = filter(stat.int.time, decades == "all decades"), doy.diff.slope ~ group)
...
# cat("\n--------------------------------------------------------------------\n")
# 
# cat("\nAnova Mean Slope of Difference of DOYs (before 1990):\n\n")
# 
# mod.int.before <- aov(data = filter(stat.int, decades == "1990s and before"), doy.diff.slope ~ group)
# 
...
# cat("\n--------------------------------------------------------------------\n")
# 
# cat("\nAnova Mean Slope of Difference of DOYs (1990 on):\n\n")
# 
# mod.int.after <- aov(data = filter(stat.int, decades == "1990s on"), doy.diff.slope ~ group)
# 
...
#plot trends of collection day in realtion to year by group
ggplot(data = dat.occ.polldep, aes(x = year, y = mean.doy, group = PollDep, color = PollDep)) +
...
#plot trends of collection day in realtion to yearly temp by group
ggplot(data = dat.occ.polldep, aes(x = y_mean_temp, y = mean.doy, group = PollDep, color = PollDep)) +
  geom_point(size = 1.5, alpha = .3, position = position_dodge(width = 0.01)) +
  geom_smooth(method = lm, se = T, size = 1.5, linetype = 1, alpha = .7) +
  theme_minimal() +
  labs(title = "Linear Relationship Between between yearly mean temperature and\nCollection Day for each Group",
...
stat.spec.time.PollDep.posthoc <- as.data.frame(stat.spec.time.PollDep.posthoc[["PollDep"]]) %>%
  mutate(pairs = unlist(attr(stat.spec.time.PollDep.posthoc[["PollDep"]], "dimnames")[1])) %>%
...
stat.spec.temp.PollDep.posthoc <- as.data.frame(stat.spec.temp.PollDep.posthoc[["PollDep"]]) %>%
  mutate(pairs = unlist(attr(stat.spec.temp.PollDep.posthoc[["PollDep"]], "dimnames")[1])) %>%
  select(pairs, everything())
...
analyse.trait(data = mutate(dat.occ.plant, meanFlMonth = floor((FlStart + FlEnd)/2)),
              trait = meanFlMonth)
```
...
source(here("Scripts", "save_plots_from_analysis.R"))
...
save.image(here("Analysis_temp.RData"))
