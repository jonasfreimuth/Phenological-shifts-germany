...
png(here("Plots", "species_mikado.png"), width = 1500, height = 1000)
...
print(
  ggplot(dat.occ %>% 
    ...
    #           col = "gray31", alpha = 0.3, size = 1.5) +
    geom_line(aes(year, min.doy, group = species),
              stat = "smooth", method = "lm", se = FALSE,
              col = "cyan", alpha = 0.3, size = 1.5) +
    geom_line(aes(year, max.doy, group = species),
              stat = "smooth", method = "lm", se = FALSE,
              col = "orange", alpha = 0.3, size = 1.5) +
    # overall regressions
    geom_line(aes(year, mean.doy, group = id.grp),
              stat = "smooth", method = "lm", se = TRUE,
              col = "black", size = 2) +
    geom_line(aes(year, min.doy, group = id.grp),
              stat = "smooth", method = "lm", se = TRUE,
              col = "blue", size = 2) +
    geom_line(aes(year, max.doy, group = id.grp),
              stat = "smooth", method = "lm", se = TRUE,
              col = "red", size = 2) +
...
print(
  dur_slope_plot_save(xlab = "Group",
                      ylab = paste("Mean slope of dedacal activity duration",
                                   "[days/decade] (\u00B1 95% CI) [days/year] (\u00B1 95% CI)",
                                   sep = "\n"))
)
...
print(
  dur_slope_plot_save(y = slope.first,
                      ...
                      ymin = ci.min.first,
                      ymax = ci.max.first,
                      xlab = "Group", ylab = "Mean slope [days/year] (\u00B1 95% CI)")
)
...
# first calculate linear model results for doy.diff ~ decade
lm.res.int <- lm.sum(int.spec.dec, group, doy.diff ~ decade) %>%
  mutate(breaks = cut(pval, breaks = c(0, 0.05, 1), labels = c("p < 0.05", "p > 0.05"),
                      right = FALSE),
...
print(
  ...
  ggplot(int.mean.time,
    ...
    geom_errorbar(aes(ymin = ci.min.doy.diff, ymax = ci.max.doy.diff),
                  size = 1.5,
                  width = 4,
                  alpha = 0.7) +
    geom_point(size = 5) +
...
print(
  ...
  ggplot() +
    ...
    geom_errorbar(data = int.mean.time,
                  aes(decade, ymin = ci.min.doy.diff,
                      ymax = ci.max.doy.diff,
                      col = group),
                  size = 1.2) +
    geom_point(data = int.mean.time,
               aes(decade, mean.doy.diff, col = group),
               size = 8) +
    geom_line(data = int.mean.time,
              aes(decade, mean.doy.diff, col = group),
              size = 1.2) +
    # geom_vline(data = eqs.doy.diff, aes(xintercept = xintercept, col = group), lty = 2) +
    scale_color_manual(
      name = "Group",
      aesthetics = c("color", "fill"),
...
for (s in sort(unique(int.spec.dec$poll))) {
  ...
  print(
    ggplot(int.s.dec, aes(x = decade, y = doy.diff, group = plant)) +
      geom_hline(yintercept = 0, lty = 3, size = 1.5, col = col.line) +
      geom_line(size = 1.5, alpha = 0.1) +
      ...
      coord_cartesian(ylim = c(1.1 * max(int.spec.dec$doy.diff),
                               1.1 * min(int.spec.dec$doy.diff)))+
      theme(text = element_text(size = 40),
            axis.text.x = element_text(angle = 30, hjust = 1),
            axis.ticks = element_line(colour = col.ax, size = 1.2),
            axis.ticks.length.y.left = unit(8, "bigpts"),
...
print(
  ggplot() +
    ...
    geom_point(
      data = stat.int.meta,
      aes(x = group, y = mean.doy.diff.slope, col = group),
      size = 8,
      position = position_dodge(width = 1)
    ) +
    geom_text(
      ...
      aes(
        x = group,
        y = ypos(stat.int.time$doy.diff.slope),
        label = paste0("n = ", N.doy.diff.slope)
      ),
      size = 15,
      col = "gray31"
    ) +
    #add labels for differing factor levels
    geom_text(
      data = pairdiff(stat.int.time, doy.diff.slope ~ group),
      aes(
        x = level,
        y = ypos(stat.int.time$doy.diff.slope, frac = 0.1),
        label = letter
      ),
      size = 15,
      col = "gray31"
...
print(
  ggplot() +
    ...
    #            aes(x = group, y = doy.diff.slope, shape = decades),
    ...
    # geom_text(data = pairdiff(stat.int.time, doy.diff.slope ~ decades, Letters = letters),
