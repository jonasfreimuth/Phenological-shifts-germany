...
# enabling this will retain the huge files downloaded from gbif containing raw occurrence data
# useful when working on that part of the get_occurrence_data script, so you don't have to download
# everything again. All the files take up about 22 GB when kept.
...
# one of 
# - spec.int.slopes: plots interactions on species level (doy vs time/temp)
...
# - data.quality.assessment: plots all occurrence records of plants and pollinators in one each
#                           (not included in full, requires more than 8 GB of RAM 
#                           (right now at least))
...
#                 * doy distribution over the year for (for now only) all Butterfly species 
#                   (to estimate the degree of multivoltism)
...
# which columns to select from pruned data, could save some memory
# should only be changed in case columns are no longer needed or new ones 
# become required
data_cols <- c('kingdom', 'phylum', 'order', 'family', 'genus', 'species',
               'year', 'month', 'day', 'doy', 'decade',
               'id.grp', 'decade2', 'long', 'lat', 'temp')
...
```{r get occurrence data, eval=FALSE, include=FALSE}
...
```{r control running of the rest, eval=FALSE, include=FALSE}

# check if it is necessary to run the whole script in principle
if (file.exists(here("data", "workspace_save.RData")) & force.main == FALSE) {
  
  # now check whether there were changes in the data i.e. we had to run the occurrence getting script
...
dat.occ <- fread("data/f_occurrences_full_pruned.csv", select = data_cols,
                 showProgress = FALSE)
  
# TODO: for testing, select a random fraction of species and use those in the 
# analysis
all_species <- unique(dat.occ$species)
frac_species <- sample(all_species, length(all_species) * 0.1)
...
# TODO: this should not be necessary for this analysis
...
cor.temp <- cor.test(dat.temp$year, dat.temp$temp)
...
p.temp +
  geom_point(aes(year, temp, col = temp)) +
  geom_smooth(aes(year, temp), method = "lm")
...
plot(lm(temp ~ year, dat.temp), which = c(1, 2, 5))
...
stat.ord.time.plants <- slopes(dat.occ.plant, doy ~ year, order)

stat.ord.temp.plants <- slopes(dat.occ.plant, doy ~ temp, order)

stat.ord.time.polls <- slopes(dat.occ.poll, doy ~ year, order)

stat.ord.temp.polls <- slopes(dat.occ.poll, doy ~ temp, order)
...
lm.res.plapoll.time <- lm.sum(dat.occ, id.grp, doy ~ year) %>%
  mutate(breaks = cut(pval, breaks = c(0, 0.05, 1), labels = c("p < 0.05", "p > 0.05"),
                      right = FALSE))
lm.res.plapoll.temp <- lm.sum(dat.occ, id.grp, doy ~ temp) %>%
  mutate(breaks = cut(pval, breaks = c(0, 0.05, 1), labels = c("p < 0.05", "p > 0.05"),
                      right = FALSE))
...
lmm.doy.time <- lmer(doy ~ year * kingdom + (1|order), data = dat.occ)
...
# same again for temp
lmm.doy.temp <- lmer(doy ~ temp * kingdom + (1|order), data = dat.occ)
...
names.dur.dec <- c(names(dat.occ)[1:8], "doy", "decade")
...
```{r Decadal duration plot, eval=FALSE, fig.height=7, fig.width=10, include=FALSE}
...
```{r dur slopes stat, eval=FALSE, include=FALSE}
...
```{r Shift of duration group plots, eval=FALSE, fig.height=7, fig.width=10, include=FALSE}
...
```{r Duration vs asymmetry, eval=FALSE, include=FALSE}
...
```{r records over time, eval=FALSE, include=FALSE}
dat.n <- dat.occ %>% 
  select(year, id.grp) %>% 
...
```{r Mikado plot, eval=FALSE, fig.height=7, fig.width=10, include=FALSE}
...
# plot yearly data
ggplot(dat.occ) +
  geom_smooth(aes(year, doy, group = species),
              method = method, se = se,
  ...
  geom_smooth(aes(year, max.doy, group = species),
              method = method, se = se,
              alpha = alpha, col = "red") +
  geom_point(aes(year, doy, group = species),
             alpha = alpha, col = "black") +
...
ggplot(dat.occ.dur.dec) +
  geom_smooth(aes(decade, doy, group = species),
              method = method, se = se,
              alpha = alpha, col = "black") +
  ...
  geom_smooth(aes(decade, max.doy, group = species),
              method = method, se = se,
              alpha = alpha, col = "red") +
  geom_point(aes(decade, doy, group = species),
             alpha = alpha, col = "black") +
...
stat.spec.time.plants <- slopes(dat.occ.plant, doy ~ year,
                                tax_level = species)
stat.spec.temp.plants <- slopes(dat.occ.plant, doy ~ temp,
                                tax_level = species)
stat.spec.time.polls <- slopes(dat.occ.poll,
                               tax_level = species)
stat.spec.temp.polls <- slopes(dat.occ.poll, doy ~ temp,
                               tax_level = species)
...
mod.time <- aov(data = stat.spec.time %>% 
                  # filter(!(id.grp %in% c("Hymenoptera", "Diptera"))) %>% 
                  ungroup(),
                slope ~ id.grp)
...
mod.temp <- aov(data = stat.spec.temp %>% 
                  # filter(!(id.grp %in% c("Hymenoptera", "Diptera"))) %>% 
                  ungroup(),
                slope ~ id.grp)
...
cor.tests <-
  lapply(unique(stat.spec.both$id.grp), function(x, data = stat.spec.both) {
    ...
    # make sure data has enough finite observations for cor.test
    if (sum(!is.na(stat.grp$time.slope)) > 2 &&
        sum(!is.na(stat.grp$temp.slope)) > 2) {
...
ggplot() +
  ...
  # group sizes, correlation and stat. significance
...
stat.spec.2k.temp <-
  slopes(dat.occ.2k, formula = doy ~ temp, thr = 2)
...
dat.int <-
  read.csv(here("static_data", "plant_pollinator_interactions_2018.csv"),
           stringsAsFactors = FALSE) %>%
...
for (i in seq(1, nrow(dat.int.year))) {
  ...
  if (any(c("full", "spec.int.slopes", "spec.int.slopes.time") %in% opts)) {
    ...
    print(
      ggplot(dat.i, aes(x = year, y = doy, col = kingdom)) +
        geom_point() +
  ...
  sum.lm.i <- summary(lm(doy ~ year * kingdom, dat.i))
  
  #run an ancova to test for a difference in the slopes of plants and pollinators
  ancov.i <- anova(lm(doy ~ year, dat.i), lm(doy ~ year * kingdom, dat.i))
...
for (i in seq(1, nrow(dat.int.year))) {
  ...
  if (any(c("full", "spec.int.slopes", "spec.int.slopes.temp") %in% opts)) {
    ...
    print(
      ggplot(dat.i, aes(x = temp, y = doy, col = kingdom)) +
        geom_point() +
        geom_smooth(method = "lm", size = 1.5) +
        labs(title = toString(unique(dat.i$species))) +
        coord_cartesian(xlim = c(min(dat.occ$temp), max(dat.occ$temp)),
                        ylim = c(0, 365)) +
        theme(axis.title = element_text(size = 40),
              axis.text = element_text(size = 40),
              axis.text.x = element_text(angle = 30, hjust = 1),
  ...
  #generate model for estimating slopes of plant and pollinator
  sum.lm.i <- summary(lm(doy ~ temp * kingdom, dat.i))
  
  #run an ancova to test for a difference in the slopes of plants and pollinators
  ancov.i <- anova(lm(doy ~ temp, dat.i), lm(doy ~ temp * kingdom, dat.i))
...
dat.occ.dec <- dat.occ <- fread("data/f_occurrences_full_pruned.csv", select = data_cols,
                 showProgress = FALSE) %>%
  
  # TODO: for testing, only use the randomly selected species from the start
  filter(species %in% frac_species) %>% 
...
ggplot(# us
  ...
  # ),
  # alpha = .15) +
  geom_line(aes(x = decade, y = doy, group = species), alpha = .5) +
  geom_point(aes(x = decade, y = doy, col = id.grp)) +
...
for (i in seq(1, nrow(int.dec))) {
  ...
  int.i.mean <- int.i %>%
    ...
    # so it's PLANT - POLLINATOR doy
    summarise(doy.diff = last(doy , kingdom) - first(doy , kingdom),
              ...
              early.partner = ifelse(doy.diff/abs(doy.diff) == 1, "plant",
                                     "poll"),
  ...
  stat.int.i.time <- data.frame(
    ...
    # first, check if NAs are present, if yes, we cant say
    early.switch = if (any(is.na(int.i.mean$early.partner))) {
      FALSE
    } else if (all(int.i.mean$early.partner == int.i.mean$early.partner[1])) {
      FALSE
    } else {
      TRUE
    }
  ...
  #generate model for doy differences over time
  sum.i.temp <- summary(lm(data = int.i.mean, formula = synchrony ~ temp))
  ...
  if (any(c("full", "int.spec.dec") %in% opts)) {
    ...
    print(
      ggplot(int.i.mean, aes(x = temp, y = synchrony)) +
        geom_line(size = 1.5) +
        geom_point(size = 4) +
        geom_smooth(method = "lm") +
        labs(title = paste(unique(int.i.mean$poll),
...
ggplot(int.spec.dec,
  ...
  labs(x = "Decadal mean Temperature [\u00B0C]",
       y = "Pla - Poll Synchrony") +
  # coord_cartesian(xlim = c(min(dat.occ.dec$temp), max(dat.occ.dec$decade))) +
  facet_wrap( ~ group, nrow = 3) +
  theme_minimal()
...
ggplot() +
  geom_point(data = int.spec.dec,
             ...
             alpha = .2) +
  geom_line(data = int.spec.dec,
            aes(temp, synchrony, group = id,
                col = group),
            alpha = .1) +
  geom_errorbar(data = int.mean.temp,
                aes(temp, ymin = ci.min.synchrony,
                    ymax = ci.max.synchrony,
                    col = group),
                size = 1.2) +
  geom_point(data = int.mean.temp,
             aes(temp, mean.synchrony, col = group,
                 pch = group),
             size = 2) +
...
# percentages of insect partner earlier
ggplot(int.mean.temp, 
       aes(temp, ins.early.frac, col = group)) + 
  geom_point(size = 2) +
  geom_smooth(method = "lm") + 
  facet_wrap(~group) +
  scale_color_manual(
...
#run lmm
lmm.doy.all <- lmer(data = dat.occ, doy ~ year * temp * kingdom + (1|species))
...
lmm.doy.pla <- lmer(data = dat.occ.plant, doy ~ year * temp + (1|species))
...
#run linear mixed model
lmm.doy.poll <- lmer(data = dat.occ.poll, doy ~ year * temp + (1|species))
...
#run linear mixed model
lm.doy.polldep <- lm(data = dat.occ.polldep, doy ~ PollDep + year:PollDep) 
...
mod.polldep <- aov(doy ~ PollDep, polldep.spec.doy)
...
# plot differences
ggplot(polldep.spec.doy, aes(PollDep, doy, group = PollDep,
                                  col = PollDep)) +
  geom_jitter(alpha = .2,
              col = "gray31") +
  stat_summary(
...
stat.spec.time.PollDep <- slopes(dat.occ.polldep, doy ~ year, species, PollDep) %>%
  mutate_if(is.character, as.factor)
stat.spec.temp.PollDep <- slopes(dat.occ.polldep, doy ~ temp, species, PollDep)%>%
  mutate_if(is.character, as.factor) 
...
#plot trends of collection day in realtion to year by group
ggplot(data = dat.occ.polldep, aes(x = year, y = doy, group = PollDep, color = PollDep)) +
...
ggplot(data = dat.occ.polldep, aes(x = temp, y = doy, group = PollDep, color = PollDep)) +
...
if (any(c("full", "adiitional") %in% opts)){
  
  source(here("Scripts", "plot_occurrence_distribution.R.R"))
