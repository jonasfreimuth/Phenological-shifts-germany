...
#load libraries
# order of packages is important to avoid conflicts 
library("here")
library("beepr")
library("data.table")
library("cowplot")
...
```{r Display meta data, echo=FALSE}

# display summary of data after pruning
print(fread("data/occurrence_full_pruned_meta.csv"))
print(fread("data/occurrences_full_pruned_summary.csv"))

# display summary of average yearly data
print(fread("data/species_yearly_mean_doy_raw_based_summary.csv"))

# summary of raw occurrence data based decadal data
print(fread("data/species_decadal_mean_doy_raw_based_summary.csv"))

# summary of yearly mean based decadal data
print(fread("data/species_decadal_mean_doy_year_based_summary.csv"))

```

```{r Plot meta data, echo=FALSE}
# pruned data
ggplot(fread("data/occurrences_full_pruned_summary.csv"),
       aes(x = id.grp, y = species.n.unique, fill = id.grp)) +
  geom_col() + 
  # scale_y_log10() +
  labs(title = "Pruned data",
       subtitle = "# species across groups",
       x = "Group",
       y = "# of species",
       fill = "Group") +
...
# yearly mean data
ggplot(fread("data/species_yearly_mean_doy_raw_based_summary.csv"),
       aes(x = id.grp, y = species.n.unique, fill = id.grp)) +
  geom_col() + 
  # scale_y_log10() +
  labs(title = "Yearly mean data",
       subtitle = "# species across groups",
       x = "Group",
       y = "# of species",
       fill = "Group") +
...
# decadal mean data, raw based
ggplot(fread("data/species_decadal_mean_doy_raw_based_summary.csv"),
       aes(x = id.grp, y = species.n.unique, fill = id.grp)) +
  geom_col() + 
  # scale_y_log10() +
  labs(title = "Raw based decadal mean data",
       subtitle = "# species across groups",
       x = "Group",
       y = "# of species",
       fill = "Group") +
...
# yearly mean data
ggplot(fread("data/species_decadal_mean_doy_year_based_summary.csv"),
       aes(x = id.grp, y = species.n.unique, fill = id.grp)) +
  geom_col() + 
  # scale_y_log10() +
  labs(title = "Yearly based decadal mean data",
       subtitle = "# species across groups",
       x = "Group",
       y = "# of species",
       fill = "Group") +
...
```{r Calculating pla poll slopes, include=FALSE}

stat.ord.time.plants <- slopes(dat.occ.plant, mean.doy ~ year, order)

stat.ord.temp.plants <- slopes(dat.occ.plant, mean.doy ~ temp, order)

stat.ord.time.polls <- slopes(dat.occ.poll, mean.doy ~ year, order)

stat.ord.temp.polls <- slopes(dat.occ.poll, mean.doy ~ temp, order)

```


Plot relationships of Plants and Pollinators occurrences:  

```{r Plot group shifts}

# generate lm results for groups to get significance of slopes
lm.res.plapoll.time <- lm.sum(dat.occ, id.grp, mean.doy ~ year) %>%
  mutate(breaks = cut(pval, breaks = c(0, 0.05, 1),
                      labels = c("p < 0.05", "p > 0.05"),
                      right = FALSE))
lm.res.plapoll.temp <- lm.sum(dat.occ, id.grp, mean.doy ~ temp) %>%
  mutate(breaks = cut(pval, breaks = c(0, 0.05, 1),
                      labels = c("p < 0.05", "p > 0.05"),
                      right = FALSE))
...
ggplot() +
  ...
  geom_abline(aes(intercept = intercept,
                  slope = slope, lty = breaks, col = id.grp),
              lm.res.plapoll.time,
              size = 1.5) +
...
ggplot() +
  geom_point(data = dat.occ,
             aes(x = temp, y = mean.doy, col = id.grp,
                 alpha = n.rec), 
             size = 1.5) +
  geom_abline(aes(intercept = intercept,
                  slope = slope, lty = breaks, col = id.grp),
...
cat("Pollinator Slope: ", round(coef.time[2,1], 3), " days per year\n",
    "Plant Slope: ", round(coef.time[2,1] + coef.time[4,1], 3),
    " days per year", 
    "\n\n")

# same again for temp
lmm.doy.temp <- lmer(
  mean.doy ~ temp * kingdom + (1|order), data = dat.occ)
...
cat("Pollinator Slope: ", round(coef.temp[2,1], 3), " days per degree\n",
    "Plant Slope: ", round(coef.temp[2,1] + coef.temp[4,1], 3),
    " days per degree\n")
...
Plot relationships of Pollinator orders:  
...
ggplot() +
  ...
  labs(x = "Year", y = "DOY",
       title = paste(
         "Linear Relationship Between between year and Collection Day",
...
names.dur.dec <- c(names(dat.occ)[1:8],
                   "mean.doy", "duration", "min.doy", "max.doy")
...
plt.dur.dec <- dur_plot(
  x = decade, y = duration, col = id.grp,
  # group = species,
  title = "Duration of activity (last - first doy) over time",
  xlab = "Decade",
  ylab = "Decadal mean duration [days]")

plt.first.dec <- dur_plot(
  x = decade, y = min.doy, col = id.grp,
  # group = species,
  title = "First occurrence DOY over time",
  xlab = "Decade",
  ylab = "Decadal mean first DOY")

plt.last.dec <- dur_plot(
  x = decade, y = max.doy, col = id.grp, 
  # group = species,
...
gridExtra::grid.arrange(
  plt.dur.dec, plt.first.dec, plt.last.dec, p.temp +
    geom_point(aes(year, temp, col = temp)) +
    geom_smooth(aes(year, temp), method = "lm"))

```

```{r dur slopes stat, include=FALSE}
...
slope_dec_diff <- dur_slope_plot(
  ...
  xlab = "Group",
  ylab = "Mean slope [days/year] (\u00B1 95% CI)",
  title = paste("Group differences of shift of decadal mean first",
...
ggplot() +
  ...
  #           aes(x = mean(c(min(stat.spec.dur$ci.min.diff),
  #               max(stat.spec.dur$ci.max.diff))),
...
p.first <- dur_plot_2k(
  x = year, y = min.doy, col = id.grp,
  data = dat.occ.2k,
  title = str_glue(
    "First occurrence DOY over time ({thr.2k.min} - {thr.2k.max})"),
  ylab = "First DOY")

p.last <- dur_plot_2k(
  x = year, y = max.doy, col = id.grp,
  data = dat.occ.2k,
  title = str_glue(
    "Last occurrence DOY over time ({thr.2k.min} - {thr.2k.max})"),
  ylab = "Last DOY")
...
slope_dur <- dur_slope_plot(
  ...
  title = str_glue(paste(
    "Group shift of yearly mean duration of",
    "flight over time ({thr.2k.min} - {thr.2k.max})"
  ))
)
...
slope_first <- dur_slope_plot(
  ...
  ylab = "Mean slope [days/year] (\u00B1 95% CI)",
  title = str_glue(paste(
    "Group shift of yearly mean first day of flight",
    "over time ({thr.2k.min} - {thr.2k.max})"
    )
  )
)
...
```{r Calculating species slopes, include=FALSE}

stat.spec.time.plants <- slopes(dat.occ.plant, mean.doy ~ year,
                                tax_level = species)
stat.spec.temp.plants <- slopes(dat.occ.plant, mean.doy ~ temp,
                                tax_level = species)
stat.spec.time.polls <- slopes(dat.occ.poll,
                               tax_level = species)
stat.spec.temp.polls <- slopes(dat.occ.poll, mean.doy ~ temp,
                               tax_level = species)
...
stat.spec.2k.temp <-
  slopes(dat.occ.2k, formula = mean.doy ~ temp, thr = 2)
...
for (i in seq(1, nrow(dat.int.year))) {
  ...
  #generate model for estimating slopes of plant and pollinator
  sum.lm.i <- summary(lm(mean.doy ~ temp * kingdom, dat.i))
  
  #run an ancova to test for a difference in the slopes of plants and pollinators
  ancov.i <- anova(lm(mean.doy ~ temp, dat.i),
                   lm(mean.doy ~ temp * kingdom, dat.i))
...
dat.occ.dec <- fread(here("data", "species_decadal_mean_doy_raw_based.csv"),
                     showProgress = FALSE) %>%
...
Looking at individual species reactions over the decades:
...
#run lmm
lmm.doy.all <- lmer(data = dat.occ, mean.doy ~ year * temp * kingdom + (1|species))
...
corvif(as.data.frame(dat.occ)[,vars])
...
lmm.doy.pla <- lmer(data = dat.occ.plant, mean.doy ~ year * temp + (1|species))
...
#run linear mixed model
lmm.doy.poll <- lmer(data = dat.occ.poll, mean.doy ~ year * temp + (1|species))
...
stat.spec.time.PollDep <- slopes(dat.occ.polldep, mean.doy ~ year, species, PollDep) %>%
  mutate_if(is.character, as.factor)
stat.spec.temp.PollDep <- slopes(dat.occ.polldep, mean.doy ~ temp, species, PollDep)%>%
  mutate_if(is.character, as.factor) 
...
save.image(here("data", "workspace_save_fdata.RData"))
