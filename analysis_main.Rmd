...
for (i in seq(1, nrow(dat.int.year))) {
  ...
  dat.i <- rbind(dat.occ %>% 
                   filter(species == as.character(dat.int.year$poll[i])) %>%
                   droplevels(),
                 dat.occ %>% 
                   filter(species == as.character(dat.int.year$plant[i])) %>%
                   droplevels())
  ...
  if (any(c("full", "spec.int.slopes", "spec.int.slopes.time") %in% opts)) {
    ...
    png(here("Plots/Interactions/time",
             paste0(i, "_", paste(unique(dat.i$species), collapse = ""),
                    ".png")),
...
for (i in seq(1, nrow(dat.int.year))) {
  ...
  if (any(c("full", "spec.int.slopes", "spec.int.slopes.temp") %in% opts)) {
    ...
    # if it doesn't exist yet, create the interaction sub directory
    dir.check(here("plots/interactions/temp"))
    
    png(here("Plots/Interactions/temp",
             paste0(i, "_", paste(unique(dat.i$species), collapse = ""),
                    ".png")),
        width = 1500, height = 1000)
...
for (i in seq(1, nrow(int.dec))) {
  ...
  # TODO: This currently does not work, as there is no longer just one temp
  #   per year (not sure why we group by temp in the first place)
  int.i.mean <- int.i %>%
    ...
    summarise(doy.diff = last(doy , kingdom) - first(doy , kingdom),
              ...
              early.partner = ifelse(doy.diff/abs(doy.diff) == 1, "plant",
                                     "poll"),
              plant = last(species, order_by = kingdom),
              poll = first(species, order_by = kingdom),
              id = i,
