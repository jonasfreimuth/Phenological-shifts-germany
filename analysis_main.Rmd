...
if (file.exists(here("Data", "workspace_save.RData")) & force.main == FALSE) {
  ...
  if (! run.occ.get) {
    ...
    # stop the script
    stop(paste("Running main script not necessary, loaded old workspace and stopped.", 
               "Enable force.main at the start of the script to force the execution of the whole script.",
               sep = "\n"))
...
for (i in seq(1, nrow(int.dec))) {
  ...
  stat.int.i.time <- data.frame(group = int.dec$Group[i],
                                ...
                                early.switch = if (all(
                                  int.i.mean$early.partner == int.i.mean$early.partner[1])
                                ) {
                                  FALSE
                                } else {
                                    TRUE
