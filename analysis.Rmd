...
if (!test_run && !run.models.ind) {
  ...
  mod_form_file <- "static_data/model_formulas_test.txt"
}
...
species_n <- fread("data/n_species_pruned_sum.csv") %>%
...
slopes_temp_sum <- slopes_temp %>% 
  ...
  # adjust diff zero pval for multiple testing
...
ggplot(slopes_all,
  ...
  geom_text(data = slopes_all_sum,
            aes(x     = mean(c(max(slopes_all$n), min(slopes_all$n))),
                y     = ypos(slopes_all$slope_temp),
                label = paste0("r = ", round(slope_n_corr_temp,    2), ", ",
                               "p = ", round(slope_n_corr_p_temp,  2))),
            col = col.annot,
            hjust = 1) +
  ...
  labs(x = "Species n", 
       ...
       subtitle = paste("Overall:", n_temp_corr_str)) +
...
ggplot(slopes_all,
  ...
  labs(x = "Species n", 
       ...
       title = "Shift vs sample size",
       subtitle = paste("Overall:", n_year_corr_str)) +
...
# disable plotting as this one takes too long
...
if (save.plots) {
  ...
  save_plot("plots/slopes_correlation.svg",
  ...
  # earlyfrac_grp_year_line_plt <- earlyfrac_grp_year_line_plt +
  #   theme(strip.text = element_blank())
  ...
  # save plot as svg and png
  save_plot("plots/interactions_all.svg", asyn_pic,
            nrow = 3,
            bg = "transparent")
  ...
  save_plot("plots/ins_early_temp.png",  earlyfrac_grp_temp_line_plt,
           bg = "transparent")
...
n_rec <- fread("data/n_records_by_idgrp_pruned.csv",
                   showProgress = FALSE) %>%
...
body_add_flextable(doc_file, slopes_n_rec_tab) %>% 
  print("plots/slopes_nrec_tab.docx")
...
asyn_tab <- int_slopes_asyn_sum %>% 
  ...
  select(
    ...
    "# Pairwise interactions"                 = n_int,
    "# Plant species"                         = n_plant,
    "# Pollinator species"                    = n_poll,
    "Temporal shifts [days / decade]"         = slope_year_asyn_mean,
    "Temperature sensitivities [days / 1 °C]" = slope_temp_asyn_mean) %>% 
